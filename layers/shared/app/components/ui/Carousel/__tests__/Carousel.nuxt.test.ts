...
describe('Carousel', () => {
  it('renders every slide passed in the default slot', async () => {
    const wrapper = await mountSuspended(Carousel, {
      props: { slideCount: 3 },
      slots: {
        default: () => [
          h(SwiperSlide, null, () => 'Slide 1'),
          h(SwiperSlide, null, () => 'Slide 2'),
          h(SwiperSlide, null, () => 'Slide 3')
        ]
      }
    })
  ...
  it('does not render navigation buttons when navigation is off', async () => {
    const wrapper = await mountSuspended(Carousel, {
      props: { slideCount: 3 },
      slots: { default: () => h(SwiperSlide, null, () => 'Slide 1') }
    })
  ...
  it('positions the track according to the current slide', async () => {
    const wrapper = await mountSuspended(Carousel, {
      props: { slideCount: 3, modelValue: 2 },
      slots: { default: () => h(SwiperSlide, null, () => 'Slide 1') }
    })

    expect(wrapper.findComponent(Swiper).props('initialSlide')).toBe(2)
  })
})
